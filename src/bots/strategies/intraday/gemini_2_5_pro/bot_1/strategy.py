"""Estrategia del Bot 1 (INTRADAY Gemini 2.5 Pro).
...
from src.bots.strategies.intraday.gemini_2_5_pro.bot_1.intraday_indicators import (
...
class IntradayBot1Strategy(BaseBotOperations):
    """Estrategia de Bot 1 - INTRADAY con Gemini 2.5 Pro.
    
    Esta estrategia implementa operaciones intradía utilizando Gemini 2.5 Pro
    ...
    def __init__(self, config: BotConfig) -> None:
        ...
        Args:
            config: Configuración del bot con parámetros de Gemini 2.5 Pro
        """
    ...
    def _prepare_intraday_data_for_ai(
        ...
        system_prompt_path = self.prompts_dir / "intraday_gemini_2_5_pro_bot_1_system.txt"
        user_prompt_path = self.prompts_dir / "intraday_gemini_2_5_pro_bot_1_user.txt"
...
✅ Puedes evaluar nuevas oportunidades de entrada (COMPRAR/VENDER) si hay setup válido."""
    ...
    def execute_cycle(self, symbol: str) -> Dict[str, Any]:
        ...
        # 2. Consultar Gemini 2.5 Pro
        self.logger.info(
            f"Consultando Gemini 2.5 Pro para {symbol}",
            extra={
                "symbol": symbol,
                "operation_id": operation_id,
                "has_active_position": has_active_position,
    ...
    def parse_ai_response(self, response_text: str) -> Dict[str, Any]:
        ...
        Convierte la respuesta en texto de Gemini 2.5 Pro a un formato
        estructurado que el bot puede ejecutar.
        
        Gemini 2.5 Pro retorna JSON con la siguiente estructura esperada:
        {
    ...
    def _get_initial_sl_from_db(self, symbol: str) -> Optional[float]:
        """Recupera SL inicial usando magic v2 de la posición activa (fallback legacy)."""
        try:
            ...
            if positions:
                ...
                for pos in positions:
                    try:
                        ...
                        # Fallback legacy prefijo bot_id
                        try:
                            if str(pos.magic).startswith(str(self.config.bot_id)):
                                current_magic = int(pos.magic)
                                break
                        except Exception:
                            continue
            ...
            if operation and getattr(operation, 'stop_loss_initial', None):
                return float(operation.stop_loss_initial)
            return None
        except Exception as e:
            self.logger.warning(f"Error obteniendo SL inicial desde BD para {symbol}: {e}", extra={"symbol": symbol, "error": str(e)})
            return None
    ...
    def _get_current_position_info(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calcular duración (manejar ausencia de time_open/time)
            open_time_raw = None
    ...
    def _execute_open_position(self, symbol: str, decision: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Verificar símbolo y extraer especificaciones
            try:
                symbol_info = self.mt5_connection.get_symbol_info(symbol)
                if symbol_info is None:
                    self.logger.error(f"Símbolo {symbol} no disponible en MT5")
                    return
            ...
            # Calcular lote dinámico según riesgo IA/config
            ...
            request = OrderRequest(
                ...
                magic=magic_v2,
                # Comentario corto y alfanumérico (MT5 <31 chars)
            ...
            if not result or not getattr(result, 'order', None):
                ...
                self.logger.info(
                    ...
                    extra={"ticket": result.order, "price": result.price, "volume": result.volume, "magic_v2": magic_v2, "risk_pct": risk_pct, "risk_amount": risk_amount}
                )
            ...
            # Seleccionar la posición por magic v2
            position_match = None
            for pos in positions:
            ...
            operation = self.operations_repo.create_operation(
                magic_number=position.magic,  # Magic v2
                bot_id=self.config.bot_id,
    ...
    def _execute_update_position(self, symbol: str, decision: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Buscar operación por ticket (persistido en BD)
