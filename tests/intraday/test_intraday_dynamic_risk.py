"""Test validación de riesgo dinámico Intraday Gemini 3 Pro.
...
Ejecutar:
    python tests/intraday/test_intraday_dynamic_risk.py
"""
...
SL_PIPS = 150   # Distancia SL ampliada para evitar stop level mínimo
TP_PIPS = 300   # Objetivo proporcional


def build_config() -> BotConfig:
    return BotConfig(
        bot_id=3,  # Bot ID actualizado (Gemini 3 Pro Intraday)
        ...
        log_level="INFO",
        save_prompts=False,
        risk_per_trade=0.8,  # fallback si IA no provee riesgo_porcentaje
    )
...
def main():
    ...
    print("=== PRE-CÁLCULO ESPERADO (Gemini 3 Pro) ===")
    ...
    print("\n✅ Test riesgo dinámico Gemini 3 Pro completado con éxito.")
